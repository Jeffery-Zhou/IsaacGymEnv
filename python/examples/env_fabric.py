...
# initialize gym, a proxy of gymapi
...
sim_params.dt = 1.0 / 60.0 # Step size
...
# --- test the args ---
print("--- args: --- \n", args)
#  Namespace(compute_device_id=0, flex=True, graphics_device_id=0, num_threads=0, physics_engine=SimType.SIM_FLEX, physx=False, pipeline='gpu', sim_device='cuda:0', sim_device_type='cuda', slices=0, subscenes=0, use_gpu=True, use_gpu_pipeline=True)
print("--- sim_params ---\n", sim_params)
# --- end the test ---

sim = gym.create_sim(args.compute_device_id, args.compute_device_id, args.physics_engine, sim_params)
# First arg to set physics simulation, and 2nd arg is to set rendering.
# physics engine, The Flex backend offers soft body and rigid body simulation that runs entirely on the GPU, but it does not fully support the tensor API yet.
if sim is None:
...
asset_root = "../../assets"
soft_asset_file = "urdf/env_fabric.urdf"
...
cam_pos = gymapi.Vec3(4.0, 4.8, 3.2)
cam_target = gymapi.Vec3(0.0, 1.4, 0.0)
